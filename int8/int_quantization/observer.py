...
class _ObserverBase(ObserverBase):
    ...
    def __init__(self, dtype=torch.quint8, qscheme=torch.per_tensor_affine,
                 bitwidth=8, log_scale=False, squeeze=0.0):
        ...
        assert (self.bitwidth <= 8 and self.bitwidth > 1), "Bitwidth should be in [2, 8]"
        if (log_scale):
            #assert (bitwidth <= 4), "Bitwidth should be <= 4 for log quantization"
            #assert (dtype==torch.qint8), "Only support qint8 dtype for log quantization"
            assert (self.qscheme in (torch.per_tensor_symmetric, torch.per_channel_symmetric)), "Only support symmetric log quantization" 
    ...
    def _calculate_qparams(self, min_val, max_val):
        ...
        #narrow range => total 2**bitwidth-1 possibilities
        if self.dtype == torch.qint8:
            if (self.log_scale): #ex: FloatSD4 => bitwidth = 4 => max = 64
                max_exp = 2**(self.bitwidth-1) - 2
                qmin = -2**max_exp
                qmax = 2**max_exp

                if (self.squeeze > 0):
                    qmin = int(qmin*(1+self.squeeze))
                    qmax = int(qmax*(1+self.squeeze))
            else:
                qmin = -2**(self.bitwidth-1) + 1
                qmax = 2**(self.bitwidth-1) - 1
        else:
            if (self.log_scale): #ex: FloatSD4 => bitwidth = 4 => -64~64 => 0~128 / zero-point=64
                max_exp = 2**(self.bitwidth-1) - 2
                qmin = 0
                qmax = 2 * 2**max_exp
            else:
                qmin = 0
                qmax = 2**(self.bitwidth) - 2
        ...
        if self.qscheme == torch.per_tensor_symmetric or self.qscheme == torch.per_channel_symmetric:
            ...
            if self.dtype == torch.quint8:
                if (self.log_scale):
                    zero_point = zero_point.new_full(zero_point.size(), 2**max_exp)
                else:
                    zero_point = zero_point.new_full(zero_point.size(), 2**(self.bitwidth-1)-1)
        else:
...
class MinMaxObserver(_ObserverBase):
    ...
    @torch.jit.export
    def extra_repr(self):
        return "min_val={}, max_val={}, bitwidth={}, log_scale={}".format(self.min_val, self.max_val, self.bitwidth, self.log_scale)
...
class MovingAverageMinMaxObserver(MinMaxObserver):
    ...
    def __init__(self, averaging_constant=0.01, dtype=torch.quint8,
                 qscheme=torch.per_tensor_affine, bitwidth=8, log_scale=False, squeeze=0.0):
...
class PerChannelMinMaxObserver(_ObserverBase):
    ...
    def __init__(self, ch_axis=0, dtype=torch.quint8,
                 qscheme=torch.per_channel_affine, bitwidth=8, log_scale=False, squeeze=0.0):
    ...
    def extra_repr(self):
        return "min_val={}, max_val={}, bitwidth={}, log_scale={}".format(self.min_vals, self.max_vals, self.bitwidth, self.log_scale)
...
class MovingAveragePerChannelMinMaxObserver(PerChannelMinMaxObserver):
    ...
    def __init__(self, averaging_constant=0.01, ch_axis=0, dtype=torch.quint8,
        ...
        super(MovingAveragePerChannelMinMaxObserver, self).__init__(
            ...
            bitwidth=bitwidth, log_scale=log_scale, squeeze=squeeze)
        self.averaging_constant = averaging_constant
...
class HistogramObserver(_ObserverBase):
    ...
    def __init__(self, bins=2048, upsample_rate=128, dtype=torch.quint8,
                 qscheme=torch.per_tensor_affine, bitwidth=8, log_scale=False):
...
class PercentileObserver(_ObserverBase):
    def __init__(self, bins=2048, upsample_rate=128, percentile=0.9999, dtype=torch.quint8,
                 qscheme=torch.per_tensor_symmetric, bitwidth=8, log_scale=False):
        # bins: The number of bins used for histogram calculation.
        super(PercentileObserver, self).__init__(dtype=dtype,
                                                 qscheme=qscheme,
                                                 bitwidth=bitwidth, 
                                                 log_scale=log_scale)
        self.bins = bins
        ...
        #assert (qscheme == torch.per_tensor_symmetric), "Only support per_tensor_symmetric scheme."
        #min_val always = 0, so only use per_tensor_affine scheme after ReLU

    @torch.jit.ignore
    def _percentile_search(self):
        total_samples = self.histogram.sum()
        boundary = total_samples * self.percentile
        max_val = self.max_val

        integral_histogram = torch.cumsum(self.histogram, 0)
        for idx_bin, num in enumerate(torch.flip(integral_histogram, dims=[0])):
            if (num.item() <= boundary):
                new_max = max_val / self.bins * (self.bins - idx_bin)
                return self.min_val, new_max
    ...
    def forward(self, x_orig):
        # type: (Tensor) -> Tensor
        x = x_orig.detach().abs() #use absolute value
        min_val = self.min_val
        max_val = self.max_val
        same_values = False
        if min_val.numel() > 0 and max_val.numel() > 0:
            same_values = min_val.item() == max_val.item()
        if min_val.numel() == 0 or max_val.numel() == 0 or same_values: #initial
            min_val = torch.min(x)*0 #always zero since we used absolute value
            max_val = torch.max(x)
            self.min_val.resize_(min_val.shape)
            self.min_val.copy_(min_val)
            self.max_val.resize_(max_val.shape)
    ...
    def calculate_qparams(self):
        ...
        new_min, new_max = self._percentile_search()

        return self._calculate_qparams(new_min, new_max)

    def _save_to_state_dict(self, destination, prefix, keep_vars):
        super(PercentileObserver, self)._save_to_state_dict(destination, prefix, keep_vars)
        destination[prefix + 'min_val'] = self.min_val
        destination[prefix + 'max_val'] = self.max_val
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        for name in local_state:
            ...
            if key in state_dict:
                ...
                missing_keys.append(key)
        super(PercentileObserver, self)._load_from_state_dict(state_dict, prefix, local_metadata, strict,
                                                              missing_keys, unexpected_keys, error_msgs)

# Restrict activations to be in the range (0,127)
default_observer = MinMaxObserver.with_args(bitwidth=7)
default_weight_observer = MinMaxObserver.with_args(dtype=torch.qint8, qscheme=torch.per_tensor_symmetric, bitwidth=4)
default_histogram_observer = HistogramObserver.with_args(bitwidth=7)
default_percentile_observer = PercentileObserver.with_args(bitwidth=8)
default_per_channel_weight_observer = PerChannelMinMaxObserver.with_args(dtype=torch.qint8, qscheme=torch.per_channel_symmetric, bitwidth=4)
default_dynamic_quant_observer = MinMaxDynamicQuantObserver

default_log_weight_observer = MinMaxObserver.with_args(dtype=torch.qint8, qscheme=torch.per_tensor_symmetric, log_scale=True, bitwidth=4)
default_per_channel_log_weight_observer = PerChannelMinMaxObserver.with_args(dtype=torch.qint8, qscheme=torch.per_channel_symmetric,
                                                                             log_scale=True, bitwidth=4)