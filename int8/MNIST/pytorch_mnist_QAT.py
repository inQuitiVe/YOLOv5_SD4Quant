...
def main():
    ...
    for mod in model.modules():
        if type(mod) == torch.nn.Conv2d:
            if mod.groups == mod.in_channels and global_counter == 0 :
                global_counter = 1
            elif mod.groups == mod.in_channels : #depthwise
                print(mod)
                mod.qconfig = log4_per_tensor_config
        elif type(mod) == torch.nn.Linear:
            mod.qconfig = log4_per_tensor_config

    model.body.conv2_depthwise.conv.qconfig = log4_per_tensor_config
